...
// Tries to get user last known location, if it is null, it tries to get user current location
function getUserLocation() {
            ...
            .then(
                res => {
                    if (res) {
                        ...
                        getUserAddress({ latitude: res.coords.latitude, longitude: res.coords.longitude }, dispatch)
                    } else {
                        locationServices.getCurrentLocation()
                            .then(
                                location => {
                                    dispatch(success({ permission: "granted", coords: location.coords }));
                                    getUserAddress({ latitude: location.coords.latitude, longitude: location.coords.longitude }, dispatch)
                                },
                                error => {
                                    dispatch(failure());
    ...
    function success(userLocation) { return { type: locationConstants.LOCATION_SUCCESS, userLocation } }
    function failure() { return { type: locationConstants.LOCATION_FAILURE } }
}


function getUserAddress(location, dispatch) {
