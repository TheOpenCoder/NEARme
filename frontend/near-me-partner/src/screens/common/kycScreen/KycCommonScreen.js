...
const KycCommonScreen = () => {
    ...
    useEffect(() => {
        rapydServices.listOfficialDocuments(partner.country_code)
        .then(
            res => {
                setKycNeeded(res)
            }
    ...
    const handleAddKyc = () => {
        ...
        if(userImage && kycFrontImage && (kycNeeded[kycIndex - 1].is_back_required ? kycBackImage : true)){
            ...
            const verificationObject = {
                "reference_id": "nearUser_" + partner.email,
                "ewallet": partner.wallet_id,
            ...
            .then(
                res => {
                    ...
                    console.log(res);
                    dispatch(authActions.completeRegistration(partner.id, "true"));
                    setLoading(false);
                },
                err => {
                    console.log(err);
                    dispatch(authActions.completeRegistration(partner.id, "false"));
                    setLoading(false);
                }
            )
        }
